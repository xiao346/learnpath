...
const completedJourneyStages = computed(() => journey.value?.completedStages ?? [])
const journeyHeadline = computed(() => {
  if (!journey.value?.configured) return '选择技术路线，开始做第一个网站'
  if (completedJourneyStages.value.includes('launch')) return `${projectName.value}已经完成，可以展示了`
  if (completedJourneyStages.value.length) return `${projectName.value}已经完成 ${completedJourneyStages.value.length} 个建站阶段`
  return `${projectName.value}正在等待第一行代码`
})
const journeyDescription = computed(() => completedJourneyStages.value.includes('launch')
...
<template>
  ...
  <div v-else-if="dashboard.data" class="dashboard-grid">
    <section class="build-focus-card glass-card"><div class="build-focus-copy"><span class="eyebrow"><i></i> {{ journey?.configured ? '我的建站进度' : '从这里开始' }}</span><h2>{{ journeyHeadline }}</h2><p>{{ journey?.configured ? journeyDescription : '告诉我们你想做什么、想用什么技术，知途会为你排好每一步。' }}</p><RouterLink class="primary-link" to="/courses">{{ journey?.configured ? completedJourneyStages.includes('launch') ? '查看完整路线' : '继续建站' : '生成我的路线' }} <span>→</span></RouterLink></div><div class="build-window" aria-hidden="true"><div><i></i><i></i><i></i></div><code><b>&lt;h1&gt;</b>你好，Web！<b>&lt;/h1&gt;</b><span>/* 你的作品从这里开始 */</span></code></div></section>
