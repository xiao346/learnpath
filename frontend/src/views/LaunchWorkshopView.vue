...
import { completeJourneyStage } from '../services/journey'
...
let journey: JourneyConfig = { project: 'portfolio', frontend: 'vue', backend: 'java', database: 'mysql' }
try {
  journey = JSON.parse(localStorage.getItem('learnpath_web_journey') ?? 'null') as JourneyConfig ?? journey
} catch {
  localStorage.removeItem('learnpath_web_journey')
}

const projectName = projectNames[journey.project] ?? '我的第一个网站'
const hasBackend = journey.backend !== 'later'
const stack = computed(() => [
  technologyNames[journey.frontend] ?? journey.frontend,
  hasBackend ? technologyNames[journey.backend] ?? journey.backend : null,
  hasBackend ? technologyNames[journey.database] ?? journey.database : null,
].filter(Boolean).join(' · '))

const checks = computed(() => [
  { id: 'build', area: '构建', title: '生产构建可以完成', detail: journey.frontend === 'vue' ? '运行 npm run build，没有类型和构建错误。' : '首页文件、样式和脚本都能被浏览器正确读取。', command: journey.frontend === 'vue' ? 'npm run build' : 'open index.html' },
  { id: 'content', area: '内容', title: '访客一眼知道你是谁', detail: `首页清楚说明${projectName}的主题，并提供至少一个真实内容区。`, command: 'check homepage' },
  ...
  hasBackend
    ? { id: 'api', area: '接口', title: '后端健康检查通过', detail: `确认 ${technologyNames[journey.backend]} 服务能接收请求并返回成功响应。`, command: 'GET /api/public/status' }
    : { id: 'links', area: '链接', title: '站内链接都能打开', detail: '逐个检查导航、作品卡片和联系方式，不留下空链接。', command: 'check all links' },
  hasBackend
    ? { id: 'data', area: '数据', title: '刷新后数据仍然存在', detail: `新增一条内容后刷新页面，确认数据已保存到 ${technologyNames[journey.database]}。`, command: 'create → refresh → read' }
...
function finishJourney() {
  if (!allPassed.value || !promiseChecked.value) return
  completeJourneyStage('launch')
...
function downloadReport() {
  ...
  link.href = url
  link.download = `${projectName}-上线检查报告.md`
  link.click()
...
<template>
  <section class="launch-workshop-page">
    ...
    <header class="launch-hero glass-card">
      ...
      <div class="launch-score" :class="{ ready: allPassed }"><small>项目健康度</small><strong>{{ Math.round(passedCount / checks.length * 100) }}</strong><span>/ 100</span><i></i></div>
    </header>
    ...
    <div v-if="!completed" class="launch-grid">
      ...
      <aside class="launch-side">
        ...
        <section class="lesson-checklist glass-card"><span>毕业确认</span><h3>{{ allPassed ? '所有检查都通过了' : '完成左侧项目体检' }}</h3><label><input v-model="promiseChecked" type="checkbox" :disabled="!allPassed" /><i></i><span>我可以向别人介绍这个网站的主题、技术路线和一个自己解决的问题</span></label><button type="button" :disabled="!allPassed || !promiseChecked" @click="finishJourney">完成我的建站之旅</button></section>
      </aside>
    </div>
