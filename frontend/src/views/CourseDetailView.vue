...
async function loadCourse() {
  ...
  try { course.value = await api<CourseDetail>(`/api/courses/${route.params.id}`) }
    ...
    <template v-else-if="course">
      ...
      <div class="detail-columns">
        <div class="detail-content-stack">
          <section class="chapter-panel glass-card"><div class="panel-title"><div><span class="mini-icon blue">▤</span><h3>课程章节</h3></div><span>{{ course.completedLessons }}/{{ course.totalLessons }} 已完成</span></div><div class="chapter-list"><article v-for="chapter in course.chapters" :key="chapter.id" :class="{ completed: chapter.completed, current: nextChapter?.id === chapter.id }"><span class="chapter-index">{{ chapter.completed ? '✓' : String(chapter.orderIndex).padStart(2, '0') }}</span><div><strong>{{ chapter.title }}</strong><small>{{ nextChapter?.id === chapter.id ? '继续学习' : `约 ${chapter.durationMinutes} 分钟` }}</small></div><span class="chapter-state">{{ chapter.completed ? '已完成' : nextChapter?.id === chapter.id ? '进行中' : '未开始' }}</span></article></div></section>
          <section class="resource-panel glass-card"><div class="panel-title"><div><span class="mini-icon cyan">↗</span><h3>精选学习资源</h3></div><span>{{ course.resources.length }} 项权威资料</span></div><div class="resource-list"><a v-for="resource in course.resources" :key="resource.id" :href="resource.url" target="_blank" rel="noopener noreferrer"><span class="resource-type">{{ resource.resourceType }}</span><div><strong>{{ resource.title }}</strong><small>{{ resource.provider }}</small><p>{{ resource.description }}</p></div><i>↗</i></a></div></section>
        </div>
