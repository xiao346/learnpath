...
const steps = computed(() => [
  ...
  {
    title: '从访客视角检查',
    command: `打开 https://${projectFolder.value}.example.site`,
    explanation: '上线成功不只看“发布完成”，还要重新打开地址，检查首页、手机布局、链接和控制台。',
    output: `✓ 网站可访问：https://${projectFolder.value}.example.site`,
  },
])
    ...
    <div class="publish-grid">
      ...
      <aside class="publish-side">
        ...
        <section class="lesson-checklist glass-card">
          ...
          <button type="button" :disabled="!canFinish || saving" @click="finishWorkshop">{{ saving ? '正在保存到数据库…' : completed ? '发布站已保存 ✓' : '完成发布站' }}</button><small v-if="error" class="practice-error">{{ error }}</small>
