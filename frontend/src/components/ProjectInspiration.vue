...
<template>
  <section class="project-inspiration" aria-label="选择想做的作品">
    <div class="goal-choices"><button v-for="(item, id) in projectGoals" :key="id" type="button" :aria-pressed="modelValue === id" :class="{ selected: modelValue === id }" @click="emit('update:modelValue', String(id))"><strong>{{ item.name }}</strong><span>{{ item.purpose }}</span><small>{{ modelValue === id ? '✓ 当前想做的作品' : '看看这个作品 →' }}</small></button></div>
...
<style scoped>
.project-inspiration { margin: 24px 0; }.goal-choices { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 12px; }
.goal-choices button { display: grid; gap: 10px; padding: 20px; border: 1px solid var(--border); border-radius: 14px; background: transparent; color: var(--text, #18324a); text-align: left; cursor: pointer; }
.goal-choices button.selected { border-color: #628bab; background: #e3edf5; }.goal-choices strong { font-size: 17px; }.goal-choices span { color: var(--muted); font-size: 14px; line-height: 1.7; }.goal-choices small { color: #376c96; }
...
.demo-items { display: grid; gap: 9px; }.demo-items article { border: 1px solid #e0e8ee; border-radius: 10px; overflow: hidden; }.demo-items button { display: flex; justify-content: space-between; align-items: center; gap: 14px; padding: 14px; border: 0; width: 100%; background: #f6f9fb; color: #29495f; text-align: left; cursor: pointer; }.demo-items small { display: block; font-size: 11px; color: #718a9b; margin-bottom: 5px; }.demo-items strong { font-size: 14px; }.demo-items p { margin: 0; padding: 14px; font-size: 13px; line-height: 1.8; }
aside h3 { margin: 10px 0; font-size: 20px; }aside h4 { margin: 20px 0 8px; }aside p { color: var(--muted); font-size: 14px; line-height: 1.8; }aside small { display: block; color: var(--muted); font-size: 12px; line-height: 1.8; }.demo-hint { background: rgba(88,142,177,.09); border-radius: 10px; padding: 14px; color: #376c96; font-size: 14px; line-height: 1.7; }.goal-reassurance { border-top: 1px solid var(--border); padding-top: 16px; }
